...
> either schema; the mechanism described here composes entirely from
> already-shipped primitives (`caps.network.allow` union, sequential kit
> startup, well-known-guest-path self-registration).
...
- The maintainer's stated direction (this session, 2026-09-17/18): kits should
  ...
  never a central kit rendering on a harness's behalf), and the daily-staleness
  problem should be solved by an in-sandbox refresh-with-cache-and-TTL, with a
  vendored snapshot as the deepest fallback — not by a human re-running a sync
...
3. **In-guest polling, reusing the harness's own already-standing network +
   credential trust boundary** (this ADR's decision). Approved 5-2. The two
   remaining rejections were specific and were incorporated into the design
   below rather than overridden:
...
### Layer 1 — Provider kit self-registration (no schema change)

A provider-role kit self-registers by writing a small, static JSON facts file
to a well-known guest path at its own `startup` phase:

```
/var/lib/acq/models/providers/<PROVIDER_ID>.json
```

`/var/lib/acq/` is already this codebase's established cross-kit guest
namespace (used today for the kit-env marker, `acq.backends/msb.sh:1571`) and
root-owned, so it is not agent-writable — closing the "any in-guest process can
plant a bogus provider" gap flagged by the Security Engineer vote.
...
an environment variable. Before the Layer 2 orchestrator ever reads that named
variable's value, it MUST confirm the SAME provider kit's own `spec.yaml`
`environment:` block (or its runtime-injected secret binding for that exact
service) declares that variable name. A provider kit cannot name a credential
it does not itself own. This check is mechanical (string comparison against
the provider kit's own declared name), not a trust judgment call.
...
1. Globs `/var/lib/acq/models/providers/*.json`.
2. For each discovered provider, checks a cache file with an embedded fetch
   timestamp, at a **host-mounted volume path** (not guest-local-only — see
   "Cache persistence" below), keyed by `providerId`.
3. If stale (default TTL 24h, overridable by a config-kit default —
   Tier 3, unchanged from prior design work), invokes that provider's own
   normalizer via a shared, vendored-into-this-kit bounded-fetch helper
   (timeout + response-size cap — the same contract as today's
   `fetchJsonBounded`, now with one canonical implementation instead of two
   already-drifted copies).
4. Validates the normalizer's OUTPUT (not just the fetch) against the neutral
...
5. On ANY failure (timeout, oversized, malformed, validation failure, no
   provider present) falls back to last-good cache, then to that provider's own
   vendored snapshot.
6. Aggregates every provider's neutral output into one file:
   `/var/lib/acq/models/catalog.json`.
7. Records provenance per provider (`"source": "live" | "cache" | "snapshot"`,
   with a timestamp) in the aggregate — DevEx's condition: a permanently
   degraded fallback must be visible, not silently indistinguishable from a
   fresh fetch.

This kit carries **zero vendor-specific knowledge** — it is pure orchestration
(glob, cache, invoke, validate, fall back, aggregate). That is what makes it a
...
**Cache persistence (per maintainer decision):** the cache and aggregate files
live on a **host-mounted volume**, shared across sandbox recreations for the
same user/host, not guest-local-only. A guest-local-only cache would make the
TTL meaningless across `acq run` invocations (every fresh sandbox starts cold).
The mount is a passive host-directory bind (`msb --mount-dir` /
sbx's neutral `volumes[]` primitive — see ADR 0001's `volumes` vocabulary,
`schemas/kit-hybrid-v1.schema.json:181-201`), not host-executed code — it does
not reintroduce the "third-party code executes on the host" problem that
Option 2 was rejected for. `acq` itself contributes no new code to make this
mount exist beyond wiring the models-kit's declared volume through the
existing neutral `volumes[]` translation, which both backends already support.
...
- `/var/lib/acq/models/catalog.json` (Layer 2's neutral aggregate), and
...
- **Env-var-ownership binding** (Contrarian): enforced as described in Layer 1.
...
- **Atomicity.** Facts, cache, and aggregate writes are temp-file + rename;
...
- `pi-coding-agent` and any future harness kit adopt this as a clean, greenfield
  ...
  burden.
- Model lists refresh daily without a human re-running a sync script and
  cutting a kit release, while remaining fully functional and deterministic
  offline (fall back to cache, then vendored snapshot).

### Negative / risks

- New moving parts: a host-mounted cache volume, a new orchestrator kit, a
  versioned schema contract between N producers and M consumers. Mitigated by
  strict layering (each layer independently testable and independently
  shippable) and by the versioned-not-negotiated schema decision above, which
  bounds the coordination cost to "rare, documented, coordinated bump" rather
  than "permanent negotiation machinery."
- Sandbox startup now has a conditional outbound step (the models kit's
  refresh-if-stale check) on the critical path. Bounded by per-provider timeout
  + a total wall-clock budget, and by the cache making the common case (fresh
  cache) a fast local read, not a network round-trip.
- Rendered harness config is no longer a pure function of pinned kit refs alone
  — it also depends on cache state at boot time. Accepted, with the fallback
  ladder and provenance recording making any degraded state visible rather
  than silent.
...
- Requires no change to the `hybrid/v1` schema itself — the entire mechanism
  ...
  sequential startup-phase ordering, well-known-guest-path self-registration,
  the neutral `volumes[]` vocabulary for the cache mount).
...
- ADR 0001 (isolation) — neutral `hybrid/v1` acq-kits spec; `volumes[]`
  vocabulary this ADR reuses for the cache mount.
