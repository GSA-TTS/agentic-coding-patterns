...
| Your working shell | An `exec zsh` line in a `99-` drop-in, so it sorts last. The loop's interactive guard (`case $- in *i*)`) is required, or the drop-in hijacks scripted `bash -lc` runs. Also guard the `exec` on a terminal (`[ -t 0 ]`): `bash -ic` is interactive but has no terminal when a script runs it. zsh does not read `~/.bashrc`: give `~/.zshrc` its own loop with a second append-if-absent step. |
