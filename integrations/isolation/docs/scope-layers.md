...
| Your working shell | An `exec zsh` line in a `99-` drop-in, so it sorts last. The loop's interactive guard (`case $- in *i*)`) is required, or the drop-in hijacks scripted `bash -lc` runs. Also guard the `exec` on `BASH_EXECUTION_STRING` being empty: `bash -ic '...'` is interactive, with or without a terminal, and bash sets that variable whenever it runs a command string. zsh does not read `~/.bashrc`: give `~/.zshrc` its own loop with a second append-if-absent step. |
