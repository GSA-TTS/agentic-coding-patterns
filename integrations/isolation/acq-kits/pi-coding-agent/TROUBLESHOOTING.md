...
need any `PATH=` prepend to find `pi` — the install prefix is `$HOME/.local`,
already first on the guest's default `PATH` (see
...
```bash
acq exec <sandbox> -- sh -c 'cat ~/.local/state/pi-coding-agent/install.log 2>/dev/null | tail -n 40'
...
roots — it needs the sandbox's proxy CA. Per the wrapper design in
`docs/decisions/`, the CA bundle is **rebuilt once per boot** by the startup
script, and the wrapper at `$HOME/.local/bin/pi` simply `exec`s the real `pi`
binary with that boot's already-current bundle path — it does not rebuild
anything itself at invocation. A plain `export` inside the install script's
own process would not persist to a separately-launched `pi` process later,
which is exactly why the wrapper exists.

**Fix:** confirm the wrapper still references `NODE_EXTRA_CA_CERTS` and
`exec`s the real binary (this is the normal, expected shape — not a sign of
breakage):

```bash
acq exec <sandbox> -- sh -c 'cat ~/.local/bin/pi'    # should show a small script that sets NODE_EXTRA_CA_CERTS and execs the real pi binary
```

If the wrapper is missing `NODE_EXTRA_CA_CERTS` entirely (not just a static
one-time export baked in at some earlier boot, but absent altogether), it has
regressed; see `docs/decisions/` for the intended design and file an issue.
