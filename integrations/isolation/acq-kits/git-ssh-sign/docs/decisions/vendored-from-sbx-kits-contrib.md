...
`integrations/isolation/acq-kits/git-ssh-sign/`, with attribution.
...
1. **Ported spec `schemaVersion "1"` → `"2"` → `"hybrid/v1"`.** Originally
   ported v1→v2 for consistency with the sibling sbx kits (`memory:` →
   `agentContext:`); in Phase 2 the whole set moved to the neutral acq
   `hybrid/v1` spec (commands gain a `phase` field; the key-resolution script
   moves from an inline `initFiles` blob to a `files/` payload). Behavior is
   identical — the same system-gitconfig install and signing-time key command.
