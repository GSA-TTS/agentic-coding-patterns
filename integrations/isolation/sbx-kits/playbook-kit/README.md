...
The kit pins the playbook with two `environment.variables` in `spec.yaml`:

- `PLAYBOOK_REF` — a human-legible release tag (e.g. `v0.12.1`), what the clone
  fetches.
- `PLAYBOOK_SHA` — the exact commit that tag must resolve to (the integrity pin).

At startup the kit shallow-clones `PLAYBOOK_REF`, then verifies the cloned
...
`PLAYBOOK_SHA` together (and the matching fallback defaults in the startup
script), then recreate sandboxes. sbx does not forward host env into startup
commands, so exporting these on the host has no effect — the values are fixed in
the kit.
